      ...
      >
        <div className="max-w-7xl mx-auto px-4 py-4 flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-4">
            <button type="button" onClick={() => setSearchOpen((p) => !p)} className="lg:hidden group" aria-label="Toggle search">
              <Search size={20} className="text-gray-700 group-hover:text-primary transition-colors" />
            </button>

            <div className="relative" ref={accountRef}>
              <button type="button" onClick={() => setAccountOpen((p) => !p)} className="text-gray-700 hover:text-primary" aria-label="Account menu">
                <User size={20} />
              </button>
              {accountOpen && (
            ...
            <button onClick={openCart} className="relative text-gray-700 hover:text-primary">
            ...
            <Link href="/wishlist" className="relative text-gray-700 hover:text-primary">
              <Heart size={20} />
            ...
            {/* Currency selector — ab flag emoji ke sath */}
            ...
            <button type="button" onClick={() => setMobileMenuOpen((p) => !p)} className="lg:hidden group" aria-label="Toggle menu">
              {mobileMenuOpen ? (
                <X size={22} className="text-gray-700 group-hover:text-primary transition-colors" />
              ) : (
                <Menu size={22} className="text-gray-700 group-hover:text-primary transition-colors" />
              )}
            </button>
          </div>
        </div>

        {/* Categories + Search row — ab pill/rounded-full shape */}
        <div className={`border-t bg-white ${searchOpen ? "block" : "hidden"} lg:block`}>
          <form onSubmit={handleSearchSubmit} className="max-w-7xl mx-auto px-4 py-3 flex items-stretch gap-3">
            <div className="relative" ref={categoryRef}>
              <button type="button" onClick={() => setCategoryOpen((p) => !p)} className="flex items-center gap-2 border border-gray-300 rounded-full pl-4 pr-3 h-full text-sm bg-white hover:border-primary focus:outline-none focus:ring-2 focus:ring-primary">
            ...
            <div className="flex items-stretch flex-1 rounded-full border border-gray-300 overflow-hidden focus-within:ring-2 focus-within:ring-primary">
              <input type="text" value={query} onChange={(e) => setQuery(e.target.value)} placeholder={t("searchPlaceholder")} className="flex-1 px-4 py-2 text-sm focus:outline-none" />
              <button type="submit" aria-label={t("search")} className="bg-primary text-white px-6 hover:bg-primary-dark transition flex items-center justify-center">
                <Search size={18} />
              </button>
            </div>
          </form>
          ...
          <div className="lg:hidden absolute top-full left-0 right-0 bg-white z-40 border-t shadow-lg max-h-[80vh] overflow-y-auto">
            <nav className="flex flex-col py-2">
              <Link
                ...
                className={`px-6 py-3 text-base font-medium transition-colors hover:bg-primary hover:text-white ${
              ...
              <div>
                <button
                  ...
                  className="w-full flex items-center justify-between px-6 py-3 text-base font-medium text-gray-800 hover:bg-primary hover:text-white transition-colors"
                >
                  {t("bestSellers")}
                  <ChevronRight
                    size={16}
                  ...
                  <div className="pb-2">
                    {categoryMenu.map((col) => (
                      <Link
                        ...
                        onClick={() => setMobileMenuOpen(false)}
                        className="block pl-10 pr-6 py-2 text-sm text-gray-600 hover:text-primary"
                      >
                        {col.title}
                      </Link>
                    ))}
              ...
              {user ? (
                <button
                  ...
                  className="flex items-center gap-2 px-6 py-3 mt-2 text-base text-gray-800 border-t text-left"
                >
                ...
                <Link
                  href="/account/login"
                  onClick={() => setMobileMenuOpen(false)}
                  className="flex items-center gap-2 px-6 py-3 mt-2 text-base text-gray-800 border-t"
                >
              ...
              <div className="flex items-center gap-4 px-6 py-4">
                <a href="#" aria-label="X" className="text-gray-600 hover:text-primary">
                ...
                <a href="#" aria-label="Pinterest" className="text-gray-600 hover:text-primary">
